...
class TestStartRun:
    ...
    @pytest.mark.asyncio
    async def test_invalid_memory_mode_is_rejected_before_run_creation(self, adapter):
        app = _create_runs_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as mock_create:
                resp = await cli.post(
                    "/v1/runs",
                    json={"input": "hello"},
                    headers={"X-Hermes-Memory": "off-ish"},
                )
                payload = await resp.json()

        assert resp.status == 400
        assert "X-Hermes-Memory" in payload["error"]["message"]
